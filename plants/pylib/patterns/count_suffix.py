...
COUNT_TERMS = TERMS.split(_COUNT_WORDS + ["dim", "count_suffix"])

_SUFFIX_TERMS = COUNT_TERMS.pattern_dict("suffix_term")
...
def on_count_suffix_match(ent):
    ...
    ent._.data[label] = COUNT_TERMS.replace.get(lower, lower)
...
def on_count_suffix_word_match(ent):
    ...
    ent._.data["low"] = t_util.to_positive_int(COUNT_TERMS.replace[word.text.lower()])
