...
PROBLEMS = set(
    """
    acre anna bahia cabrera castilla clara columbia costa cuba end garcia killip konig
    maria medium mono morales name paris ramon serra side victoria
    """.split()
)
...
def add_pattern(taxa, pattern, rank):
    if VALID_PATTERN.match(pattern) and len(pattern) >= MIN_LEN:
